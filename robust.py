...
csv_files = glob.glob('robustness/*.csv')
results = []
...
final_df['scenario_num'] = final_df['scenario'].str.extract(r'robust(\d+)').astype(int)
...
def calculate_stats(data, metric):
    stats = data.groupby('vehicle')[metric].agg(['mean', 'std'])
    stats['cv'] = (stats['std'] / stats['mean']) * 100  # Coefficient of variation
    return stats

# Calculate statistics for speed, deviation, and acceleration
speed_stats = calculate_stats(final_df, 'speed')
deviation_stats = calculate_stats(final_df, 'deviation')
acceleration_stats = calculate_stats(final_df, 'acceleration')
