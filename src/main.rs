...
impl Verlet {
    pub fn from_start_and_velocity(start: Vec2, velocity: Vec2) -> Self {
        Self { previous: start - velocity, position: start, velocity, multiplier: 1.0, ..Default::default() }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // You can change the defaults here but I would recommend using the CLI arguments
    let scenario = args.scenario.unwrap_or("obstacle0".to_string());
    let config = args.config.unwrap_or("config.json".to_string());

    let logger = Writer::from_path(format!("infrastructure/{scenario}.csv")).unwrap();
    let config = parse::<Config>(config)?;
    let scenario = parse::<Scenario>(format!("scenarios/{scenario}.json")).unwrap();
...
fn integration_step(mut query: Query<(&mut Verlet, &mut Rolling)>, cfg: Res<Config>) {
    for (mut verlet, mut rolling) in query.iter_mut() {
        ...
        verlet.acceleration = (interaction_force + verlet.driving_force + verlet.border_force + rolling.nudge) * verlet.multiplier;
...
fn make_commuter_bundle(commuter: Commuter) -> impl Bundle {
    ...
    let mut verlet = Verlet::from_start_and_velocity(commuter.start_position, velocity);

    if let Vehicle::Obstacle = commuter.vehicle {
        verlet.multiplier = 0.0;
    }
